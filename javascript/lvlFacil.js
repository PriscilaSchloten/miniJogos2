...
let desabilidaCarta = document.querySelectorAll('.desabilitar-carta');
  ...
  if (valores === 5) {
    const minutoPlayer = 2 - minuto.innerHTML;
    const segundoPlayer = 60 - timer.innerHTML;
    const dificuldade = "Facil";
    clearInterval(this.loop);
    alert(`Parabéns, ${spanPlayer.innerHTML}! Seu tempo foi de: ${2 - minuto.innerHTML}:${60 - timer.innerHTML}`);
...
const startTimer = () => {
  this.loop = setInterval(() => {
    ...
    if(timer.innerHTML > 0){
      ...
      timer.innerHTML = currentTime + 10;
    }
  }
  //checkEndGame();
  }, 1000);
